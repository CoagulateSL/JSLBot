...
public final class Circuit extends Thread implements Closeable {
	...
	@Nonnull private  Date              lastacks       =new Date();
	// last housekeeping
	@Nonnull private  Date              lastmaintenance=new Date();
	// name of the simulator
	@Nonnull private  String            regionname     ="";
	...
	Circuit(@Nonnull final JSLBot parent,
		...
		if (passedregionhandle==null) {
			throw new IllegalArgumentException("Null region handles are not allowed");
		}
	...
	public CAPS getCAPS() {
		if (caps==null) {
			throw new IllegalStateException("CAPS is not currently configured for this circuit");
		}
		return caps;
	}
	...
	@Nonnull
	public String getSimIPAndPort() {
		return simip+":"+simport;
	}
	...
	public void connect() throws IOException {
		...
		if (Debug.CIRCUIT) {
			log.finer("Sending initial circuit code over socket to "+address);
		}
		...
		if (Debug.CIRCUIT) {
			log.finer("Outstanding ACKS: "+inflight.size());
		}
		if (!inflight.isEmpty()) {
	...
	public void send(@Nonnull final Packet p) throws IOException {
		...
		if (p.getZeroCode()) {
			transmit=BotUtils.zeroEncode(transmit);
		}
		...
		if (!sending.isEmpty()) {
			...
			for (final Integer i: sending) {
				@Nonnull final U32BE acknumber=
						new U32BE(i); // yup, the appended ones are big endian.   but a PacketAck uses little endian in the body.  LL ;)
			...
			lastacks=new Date();
			if (Debug.ACK) {
				log.log(Level.FINEST,"Appended ACKS:{0}",ackedlist.toString());
			}
		...
		if (Constants.PACKET_ACCOUNTING) {
			...
			if (botcopy!=null) {
				botcopy.bytesout.addAndGet(packet.getLength());
			}
		}
		if (Constants.PACKET_ACCOUNTING_BY_MESSAGE) {
			if (botcopy!=null) {
				botcopy.accountMessageOut(p.getId(),packet.getLength());
			}
		}
		try {
			socket.send(packet);
			...
			errors++;
			if (errors>=3) {
				throw new IOException("Circuit to "+getRegionName()+" failed 3x",e);
			}
		}
		packetrate++;
	}
	...
	public JSLBot bot() {
		...
		if (mybot==null) {
			throw new NullPointerException("Bot connection has gone away");
		}
		return bot;
	}
	...
	public void run() {
			...
			while (!disconnected&&!disconnectlogged&&!bot().quitting()) {
				try {
					...
					try { p=Packet.decode(rx); }
						...
						log.log(SEVERE,"Unable to decode packet type",e);
					}
					if (p!=null) {
						if (Constants.PACKET_ACCOUNTING_BY_MESSAGE) {
							@Nullable final JSLBot botcopy=bot;
					...
					if (Debug.ACK) {
						log.finer("Exiting receive without event");
					}
				...
				if ((!ackqueue.isEmpty()&&lastAck()>2000)||ackqueue.size()>32) {
					...
					if (Debug.ACK) {
						log.finer(
								"NOT sending any ACKs.  Q size is "+ackqueue.size()+" and lastAck is "+lastAck()+"ms");
					}
				}
		...
		if (Debug.CIRCUIT) {
			log.log(Level.FINE,"Deregistering circuit to {0}",regionname);
		}
		@Nullable final JSLBot mybot=bot;
		if (mybot!=null) {
			mybot.deregisterCircuit(regionhandle,this);
		}
	}
	...
	public String toString() {
		return getRegionName()+"#"+circuitsequence;
	}
	 ...
	 */
	public void send(final Message m,final boolean reliable) throws IllegalStateException {
	...
	public void close() {
		...
		}//debug(owner,"Socket closure exceptioned - ",e); }
		if (!disconnectlogged) {
			disconnectlogged=true;
	...
	public void connectCAPS(final String newcapsurl) {
		...
		capsurl=newcapsurl;
		if (Debug.EVENTQUEUE) {
			log.log(Level.INFO,"Establishing connection to CAPS for region {0}",getRegionName());
		}
	...
	private void maintenance() {
		...
		if (interval>(Constants.CIRCUIT_PING*1000)) {
			...
			pinged=false;
		} // ping when crossing threshold.  once.  reset threshold detection here.
	...
	private void sendAck() {
		...
		if (!sending.isEmpty()) {
			...
			if (Debug.ACK) {
				log.log(Level.FINEST,"Standalone acks: {0}",acklist.toString());
			}
			send(ack);
		}
		lastacks=new Date();
	...
	private void processPacket(@Nonnull final Packet p) {
		if (Debug.PACKET) {
			log.log(Level.FINEST,"Received packet: {0}",p.dump());
		}
		...
		if (p.getReliable()) {
			...
			if (Debug.ACK) {
				log.log(Level.FINEST,"Requested to acknowledge packet {0}",p.getSequence());
			}
			synchronized(ackqueue) {
		...
		boolean internal=
				false; // circuit control packets.  only for stuff that should never be propagated.  acks and pings basically.
		...
		if (m instanceof @Nonnull final RegionHandshake r) {
			...
			final boolean register=
					false; // most connections register at creation since we know where we're connecting to
			...
			if (Debug.REGIONHANDLES) {
				log.log(Level.FINE,"RegionHandshake with UUID {0}",regionuuid.toUUIDString());
			}
			if (firsthandshake) {
				log.log(Level.INFO,
				        "Handshake {0} cpu {1}/{2} active as {3}#{4} @ colocation {5}",
				        new Object[] {regionname,r.bregioninfo3.vcpuclassid.value,r.bregioninfo3.vcpuratio.value,
				                      r.bregioninfo3.vproductname.toString(),r.bregioninfo3.vproductsku.toString(),
		...
		if (!internal) {
			...
			if (Debug.REGIONHANDLES&&regionhandle==null) {
				log.info("Creating event with null RH :( ");
			}
