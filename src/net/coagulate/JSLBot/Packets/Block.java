...
public abstract class Block {
		...
		if (Block.class.isAssignableFrom(f.getType())) {
			try {
				...
				throw new IllegalArgumentException(
						"Unable to size supposed Block "+this.getClass().getName()+"/"+f.getName()+" type "+
						f.getType().getName(),ex);
			}
		}
		if (List.class.isAssignableFrom(f.getType())) {
			try {
				@Nonnull final List<Block> l=(List<Block>)(f.get(this));
				@Nonnull final Class<?> listtype=
				...
				throw new IllegalArgumentException(
						"Unable to size supposed variable count block "+this.getClass().getName()+"/"+f.getName()+
						" type "+f.getType().getName(),e);
			}
		}
		try {
			...
			throw new IllegalArgumentException(
					"Unable to size supposed LL Type "+this.getClass().getName()+" field "+f.getName()+" of type "+
					f.getType().getName(),ex);
	...
	private List<Field> getFields() {
		...
		for (@Nonnull final Field f: fs) {
			if (!"this$0".equals(f.getName())) {
				...
				if (a==null) {
					throw new IllegalArgumentException("Field "+f.getName()+" of type "+f.getType().getName()+
					                                   " does not have annotation Sequence (?)");
				}
				map.put(a.value(),f);
			}
		}
	...
	private void fieldToBytes(@Nonnull final Field f,@Nonnull final ByteBuffer output) {
		...
		if (Type.class.isAssignableFrom(f.getType())) {
			try {
				...
				throw new IllegalArgumentException(
						"Internal error during field to bytes on field "+f.getName()+" of type "+f.getType().getName(),
						ex);
			}
		}
		throw new UnsupportedOperationException(
				"Unknown field type, field name "+f.getName()+" type "+f.getType().getName());
	}
	...
	private void writeField(@Nonnull final ByteBuffer out,@Nonnull final Field f) {
		if (Block.class.isAssignableFrom(f.getType())) {
			...
			try {
				...
				throw new IllegalArgumentException("Error writing field "+f.getName()+" of type "+f.getType().getName(),
				                                   ex);
			}
		}
		if (List.class.isAssignableFrom(f.getType())) {
			try {
				...
				throw new IllegalArgumentException(
						"Error writing variable count block "+this.getClass().getName()+"/"+f.getName()+" type "+
						f.getType().getName(),e);
			}
		}
		fieldToBytes(f,out);
	...
	private void readField(@Nonnull final ByteBuffer in,@Nonnull final Field f) {
		...
		if (debug) {
			System.out.println(
					"READING FIELD "+f.getName()+" from "+this.getClass().getName()+" type "+f.getType().getName());
		}
		if (Block.class.isAssignableFrom(f.getType())) {
			final Block b;
			try {
				...
				throw new IllegalArgumentException(
						"Error reading Block field "+f.getName()+" of type "+f.getType().getName(),ex);
			}
		}
		if (List.class.isAssignableFrom(f.getType())) {
			try {
				...
				throw new IllegalArgumentException(
						"Failed to read LIST type (variable block type) from class "+this.getClass().getName()+
						" field "+f.getName()+" type "+f.getType().getName()+" generic type "+
						f.getGenericType().getTypeName(),e);
			}
		}
