...
public abstract class Atomic {
	...
	static Atomic create(@Nonnull final Node item) {
		...
		if ("string".equals(type)) {
			return new LLSDString(item);
		}
		if ("map".equals(type)) {
			return new LLSDMap(item.getChildNodes());
		}
		if ("integer".equals(type)) {
			return new LLSDInteger(item);
		}
		if ("boolean".equals(type)) {
			return new LLSDBoolean(item);
		}
		if ("real".equals(type)) {
			return new LLSDReal(item);
		}
		if ("array".equals(type)) {
			return new LLSDArray(item.getChildNodes());
		}
		if ("uuid".equals(type)) {
			return new LLSDUUID(item);
		}
		if ("binary".equals(type)) {
			return new LLSDBinary(item);
		}
		if ("date".equals(type)) {
			return new LLSDString(item);
		}
