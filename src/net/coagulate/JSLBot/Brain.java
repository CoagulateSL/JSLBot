...
public class Brain {
	...
	@Nonnull private final Set<Handler>            brain;
	@Nonnull private final JSLBot                  bot;
	...
	@Nonnull private       Authorisation           auth;
	...
	@Nullable
	public String execute(@Nonnull final Event event) {
	...
	private String execute(@Nonnull final Event event,final boolean immediate) {
		...
		if (event instanceof UDPEvent||event instanceof XMLEvent) {
			method+=(immediate?"Immediate":"Delayed");
		}
		...
		if (event instanceof UDPEvent||event instanceof XMLEvent) {
			...
			if (!warned.contains(fen)&&(nodelayedhandler)&&(noimmediatehandler)) {
	...
	private void populateHandlerMap(@Nonnull final Event event,final String suffix) {
		...
		for (@Nonnull final Handler handler: brain) {
			try {
				if (event instanceof UDPEvent) {
					methods.add(handler.getClass().getMethod(fen,UDPEvent.class));
				}
				if (event instanceof XMLEvent) {
					methods.add(handler.getClass().getMethod(fen,XMLEvent.class));
				}
	...
	void loginLoopSafety() {
		...
		if (ago<(Constants.MAX_LAUNCH_ATTEMPTS_WINDOW_SECONDS)) {
			log.severe("Reconnection Safety: This is less than the threshold of "+
			           Constants.MAX_LAUNCH_ATTEMPTS_WINDOW_SECONDS+", tripping safety.");
	...
	private void loginLoopSafetyViolation() {
		...
		for (int i=15;i>0;i--) {
			log.severe(
					"Reconnection Safety: RECONNECTION SAFETY HAS TRIPPED.  THREAD FORCE-SLEEPING FOR "+i+" MINUTES.");
			try {
				Thread.sleep(60000);
			} catch (@Nonnull final InterruptedException ignored) {
			}
	...
	private void loadHandler(@Nullable final String handlername) {
		if (handlername==null) {
			throw new NullPointerException("No handler specified");
		}
		try {
	...
	private Handler createHandler(@Nonnull final String name) throws InvocationTargetException {
		try {
			...
			if (!name.contains(".")) {
				classname="net.coagulate.JSLBot.Handlers."+name;
			}
		...
		} catch (@Nonnull final SecurityException|
					...
					if (commandmap.containsKey(commandname)) {
						...
						if (commandname.endsWith("command")) {
							if (CommandEvent.class==m.getParameters()[0].getType()) {
								...
								log.severe("Otherwise legitimate command "+commandname+
								           " has incorrect first parameter - should be type CommandEvent");
							}
						} else {
							log.warning("Annotated command "+commandname+
							            " does not have 'command' suffix and is inaccessible");
						}
					}
				}
			}
